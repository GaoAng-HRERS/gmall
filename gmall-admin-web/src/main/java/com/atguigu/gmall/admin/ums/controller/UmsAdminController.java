 ...
 * 2、告诉SpringBoot，这个需要校验。
 *      SpringMvc进入方法之前，确定参数数值的时候就会惊醒校验，如果校验出错，直接返回错误，不执行Controller方法。
 * 3、如何感知校验成功还是失败！
 *      只需要给开启了校验的JavaBean参数后面，紧跟一个BindingResult对象就可以获取到校验结果
 *      只要有BindingResult即使校验错了，方法也会执行，我们需要手动处理。
 */
@Slf4j
@CrossOrigin
@RestController
...
public class UmsAdminController {
    ...
    public Object register(@Valid @RequestBody UmsAdminParam umsAdminParam, BindingResult result) {
        ...
        log.debug("需要注册的用户详情：{}，校验错误数：{}",umsAdminParam,errorCount);
        return new CommonResult().success(admin);
    }

    /**
     * 登录成功以后用户的信息已jwt令牌的方式返回给前端
     * @param umsAdminLoginParam
     * @param result
     * @return
     *
     * 1、如果前端发过来的是json字符创，要封装的对象。
     * public Object login(@RequestBody UmsAdminLoginParam umsAdminLoginParam）
     * 2、如果前端发过来的是k=v&k=v字符创，要封装的对象
    ...
    public Object getAdminInfo(HttpServletRequest request) {
         ...
         *2、Mybatis-plus的泛型机制跟dubbo不兼容，所以dubbo没办法直接调用Mybatis-plus中带泛型的service；
         *3、实战经验：
         *  自动生成的可用有兼容问题，最好不要远程调用；
         */
